...
| Question | Answer |
|---|---|
| **Are we losing performance today?** | Not catastrophically, but there are **two structural inefficiencies on the decode hot path**: (1) the DSPark draft (K192) is near full-size, so every speculative step runs a second, ~89%-large EXL3 Trellis MoE forward — roughly **doubling per-step weight traffic**; (2) the target decode path is a single-token `m=1` Trellis MoE with `block_m=8`, i.e. the kernel is planned for 8-row blocks. Both are tunable/configurable (no code change). |
...
| **Space for optimization?** | Yes — ranked in `03-optimization-levers.md`. The top lever is the **DSPark draft/spec-token system sweep** (K × `num_speculative_tokens`); the second is **adopting 4 small, already-upstream attention/indexer patches** the repo does not have yet (esp. `#52823` adaptive-topk-width). |
...
| TTFT | ~1.6s @ no ctx → 235.8s @ 253k ctx |
...
| `03-optimization-levers.md` | Ranked, actionable levers with expected value / effort / A-B test plan |
...
The image is well-engineered and already carries the three most impactful upstream attention hotfixes for v0.26.0 (`#49486`, `#50298`, `#50365`) plus working 216-expert fused routing. The decode hot path is dominated by the EXL3 Trellis MoE executed twice per output token (K192 draft forward + K216 target verify) at tiny `m`, which is bandwidth-inefficient by design. The fastest wins do **not** require code: sweep `DSPARK_DRAFT_EXPERTS` and `num_speculative_tokens` to maximize accepted-tokens/s, and sweep `VLLM_EXL3_TRELLIS_BLOCK_M`/prefill knobs. The fastest *code* wins are four small upstream patches the repo lacks (`#52823`, `#51967`, `#52084`, `#50911`). The `v0.28`/KV-layout refactor is not portable — it belongs to a full version bump, not a backport.
