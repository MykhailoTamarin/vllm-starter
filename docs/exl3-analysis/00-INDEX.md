...
| Question | Answer |
|---|---|
| **Are we losing performance today?** | Not catastrophically, but there are **two structural inefficiencies on the decode hot path**: (1) the DSPark draft was near full-size (K192 = ~89% of target), roughly doubling per-step weight traffic — **fixed by the K160 switch** (measured 2026-08-22); (2) the target decode path is a single-token `m=1` Trellis MoE with `block_m=8` — **the legal minimum** (plan values 8/16/32/48/64; 4 is rejected at boot), so this inefficiency is by-design and not config-addressable. Attention backports A/B'd neutral; capture-size alignment A/B'd neutral (all 2026-08-22/23 measurements). |
...
| **Space for optimization?** | Decode-side config levers are now **exhausted and verified**: K160 applied (L1); K5 spec validated (L2, REAP-pinned — optional spot-check only); Trellis `BLOCK_M` closed (8 = legal minimum); capture sizes neutral (L3); attention backports neutral (L5); **L4 verified** — shared expert correctly executed, routed-expert `swiglu_limit` clamp not applied (fidelity nit, opaque-kernel caveat, §4). Remaining: **L6** prefill knobs (low priority), kernel profile (diagnostic). |
...
| `../../models/benchmarks/<model>/draft-acceptance-sweep-2026-08-23-capsiz/` | Capture-size A/B K160 (`1 2 4 6`→`1 2 3 5 6`) — neutral |
| `../../models/benchmarks/<model>/draft-acceptance-sweep-2026-08-23-blockm4/` | `TRELLIS_BLOCK_M=4` A/B — failed to boot; planner evidence (`FAILED-blockm4-evidence.md`) |

---

## 4. One-paragraph summary

The image is well-engineered and already carries the three most impactful upstream attention hotfixes for v0.26.0 (`#49486`, `#50298`, `#50365`) plus working 216-expert fused routing. The decode hot path is dominated by the EXL3 Trellis MoE executed twice per output token (draft forward + target verify) at tiny `m`, which is bandwidth-inefficient by design. An acceptance sweep (K64→K192, coding/chat/text) showed **K160 is the sweet spot** — coding 45.3% acceptance / 39.3 t/s, text 39.7% / 35.0, chat 35.0% / 31.9 — and the server now runs **K160** (`DSPARK_DRAFT_EXPERTS=160`; K192 only wins on chat at ~89% draft cost). The three attention backports (`#52823`, `#51967`, `#52084`) are in, rebuilt, and **A/B-verified neutral** (no regression). The Trellis decode-param lever was tested and is **closed**: `BLOCK_M=4` is rejected by the SparkInfer planner (8 is the legal minimum, already served) and capture-size alignment is neutral (§3.1b). L4 verified the **shared expert is correctly executed** (FP8, runner path, own clamp) while the routed-expert `swiglu_limit=10.0` clamp is **not applied** in the visible path (fidelity nit, opaque-kernel caveat, §4). Decode-side config levers are exhausted; the remaining options are **L6** prefill knobs (low priority) and a kernel profile (diagnostic). The `v0.28`/KV-layout refactor is not portable — it belongs to a full version bump, not a backport.
