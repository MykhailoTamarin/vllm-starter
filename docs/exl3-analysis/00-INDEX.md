...
| **Space for optimization?** | Decode-side config levers are now **exhausted**: K160 applied (L1); attention backports A/B'd neutral (L5); Trellis `BLOCK_M` closed (4 rejected by planner — 8 is the legal minimum); capture sizes neutral (L3). What remains: **L2** (spec-token count/sampling 5/7/9), **L6** prefill knobs (`PREFILL_BLOCK_M`/`CHUNK`, low priority), **L4** correctness gate. A kernel profile (`nsys`/`ncu`) is the only remaining decode diagnostic. |
