...
#
# VENDORED VERBATIM (file content unmodified) from MiaAI Lab's
#   https://github.com/MiaAI-Lab/Qwen3.8-Flash-Next-Single-DGX-Spark
#   (files/patch_ple_offload.py, AGPL-3.0-or-later)
# so the edits vllm-starters applies to vllm/vllm-openai:qwen38-flash-next
# stay byte-identical to Mia's validated recipe. Only the staging differs:
# pristine sources are taken from the running image's installed vLLM instead
# of `docker cp`-extracted *.orig files, and the FP8-aware _attach_packed_table
# swap is applied by models/files/patch_qwen38_flash_next_nvfp4.py afterwards.
