...
class BenchmarkConfig(BaseModel):
    ...
    idle_wait: bool = Field(
        False,
        description="Wait for vLLM to become idle between test configurations",
    )
    idle_interval: int = Field(
        2,
        description="Seconds between /metrics checks when idle waiting",
    )
    idle_max_retries: int = Field(
        300,
        description="Max retries before timeout when idle waiting (default: 300 = 10 minutes)",
    )
    ...
    def from_args(cls):
        ...
        parser.add_argument(
            "--idle-wait",
            action="store_true",
            help="Wait for vLLM to become idle between test configurations (polls /metrics)",
        )
        parser.add_argument(
            "--idle-interval",
            type=int,
            default=2,
            help="Seconds between /metrics checks when idle waiting (default: 2)",
        )
        parser.add_argument(
            "--idle-max-retries",
            type=int,
            default=300,
            help="Max retries before timeout when idle waiting (default: 300 = 10 minutes)",
        )
        ...
        return cls(
            ...
            idle_max_retries=args.idle_max_retries,
