...
Ported from MiaAI-Lab/GLM-5.3-Flash-EXL3-2x-DGX-Sparks
(overlay/patch_xgrammar_termination.py) — the source-exact upstream
backports are model-agnostic.

The v0.26.0 base predates two merged XGrammar fixes.  Its backend can
continue feeding tokens to a matcher after an EOS/stop token terminates it.
That is especially visible with a multi-token speculative batch: the first
trailing token is rejected, subsequent advances can warn, and the cached
termination flag can become inconsistent with the matcher.
...
MANAGER_TARGET = Path(
    os.environ.get(
        "VLLM_XGRAMMAR_MANAGER_PY",
        "/usr/local/lib/python3.12/dist-packages/vllm/v1/structured_output/"
        "__init__.py",
    )
)
BACKEND_MARK = (
    "    # [xgrammar-termination] Source-exact vLLM 12f64b39 backport.\n"
)
MANAGER_MARK = (
    "                    # [xgrammar-reasoning] Source-exact vLLM "
    "c6e19b3 backport.\n"
)
...
def replace_file(target: Path, source: str) -> None:
    tmp = target.with_name(f".{target.name}.xgrammar.tmp")
    try:
