...
def _make_exl3_ext_stub() -> Any:
    class _Exl3ExtStub:
        exl3_gemm = staticmethod(lambda *a, **kw: (_ for _ in ()).throw(RuntimeError("exllamav3_ext stub")))
        exl3_moe = staticmethod(lambda *a, **kw: (_ for _ in ()).throw(RuntimeError("exllamav3_ext stub")))
        exl3_moe_fused = staticmethod(lambda *a, **kw: (_ for _ in ()).throw(RuntimeError("exllamav3_ext stub")))
        @staticmethod
        def exl3_moe_max_concurrency(device):
            return 4
    return _Exl3ExtStub()
...
class Exl3Config(QuantizationConfig):
    ...
    def __init__(
        ...
        # The checkpoint's non-EXL3 linears (attention QKV/O, shared experts,
        # indexer) are FP8 block-quantized.  Expose the attributes the
        # Fp8LinearMethod reads from `quant_config` so those linears route to
        # Fp8LinearMethod instead of silently loading raw FP8 bytes as BF16
        # (which produced garbage).  See get_quant_method().
        bqc = base_quantization_config or {}
        self._base_quantization_config = bqc
        self.weight_block_size: list[int] | None = bqc.get("weight_block_size")
        self.activation_scheme: str | None = bqc.get("activation_scheme", "dynamic")
        scale_fmt = bqc.get("scale_fmt", "fp8")
        self.is_scale_e8m0: bool = scale_fmt == "ue8m0"
        # deep_gemm not used for the SM121 EXL3 path; let Fp8LinearMethod
        # fall back to its own detection when unset.
    ...
    def _maybe_load_quantization_config(
        self,
        model_name: str,
        revision: str | None,
    ...
    def _hydrate_fp8_attrs(self, config: dict[str, Any]) -> None:
        bqc = config.get("base_quantization_config") or {}
        if bqc.get("weight_block_size") is not None:
            self.weight_block_size = bqc["weight_block_size"]
        if bqc.get("activation_scheme") is not None:
            self.activation_scheme = bqc["activation_scheme"]
        scale_fmt = bqc.get("scale_fmt", "fp8")
        self.is_scale_e8m0 = scale_fmt == "ue8m0"
    ...
    def maybe_update_config(
        ...
        if (
            ...
            cfg = self._maybe_load_quantization_config(model_name, revision,
                                                       hf_config)
            if cfg is not None:
                # Populate FP8 storage entries AND FP8 attributes from
                # quantization_config.json (the rank-sliced hybrid_tr3_tail
                # metadata only covers EXL3 trellis; FP8 attention, shared
                # experts, and indexer entries live in the full config).
                ts = cfg.get("tensor_storage")
                if ts:
                    self.tensor_storage = {**self.tensor_storage, **ts}
                self._hydrate_fp8_attrs(cfg)
            return
        ...
        if not self.tensor_storage:
            config = self._maybe_load_quantization_config(model_name, revision,
                                                          hf_config)
            ...
            self._hydrate_fp8_attrs(config)
    ...
    def _configure_rank_sliced(self, metadata: dict[str, Any]) -> None:
        ...
        if metadata["tensor_schema"] not in expected_schemas:
            raise ValueError(
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase) or is_lm_head:
            # EXL3 trellis returns Exl3LinearMethod.
            if self._linear_prefix_is_exl3(prefix):
                return Exl3LinearMethod(self)
            # FP8 block-quantized linears (attention QKV/O projections, shared
            # experts, indexer) must go through Fp8LinearMethod so the raw
            # float8 weights are dequantized with their E8M0 scales.  Routing
            # them to UnquantizedLinearMethod previously loaded raw FP8 bytes
            # as BF16 numbers, producing garbage output.
            if self._linear_prefix_is_fp8(prefix):
                return Fp8LinearMethod(self)
            return UnquantizedLinearMethod()
        if isinstance(layer, RoutedExperts):
            if not self._moe_prefix_is_exl3(prefix, layer):
                # Non-EXL3 MoE layers (e.g. the embedded DSPark MTP draft whose
                # experts are stored native fp4-packed, not EXL3 trellis) must
                # go through Mxfp4MoEMethod so the packed I8 weights load with
                # their ue8m0 scales.  Falling through to None makes vLLM pick
                # UnquantizedFusedMoEMethod, which allocates full BF16 expert
                # buffers and both OOMs and misloads the FP8 bytes.
                return Mxfp4MoEMethod(layer.moe_config)
            return Exl3MoEMethod(self, layer.moe_config)
        return None

    def _fp8_moe_config(self) -> Fp8Config | None:
        """Build (and cache) an FP8 config for the non-EXL3 MoE layers.

        The checkpoint's base_quantization_config describes the FP8 block
        quantization used by the attention linears, shared experts, and the
        embedded DSPark draft experts.  Reuse it so Fp8MoEMethod's block-scaled
        expert loading matches the stored format.
        """
        if getattr(self, "_cached_fp8_moe_config", None) is not None:
            return self._cached_fp8_moe_config
        bqc = getattr(self, "_base_quantization_config", None)
        if not bqc:
            return None
        cfg = Fp8Config(
            is_checkpoint_fp8_serialized=True,
            activation_scheme=bqc.get("activation_scheme", "dynamic"),
            weight_block_size=bqc.get("weight_block_size"),
        )
        self._cached_fp8_moe_config = cfg
        return cfg
    ...
    def _storage_entry(self, prefix: str) -> dict[str, Any] | None:
        ...
        # Embedded DSPark MTP draft layers: the checkpoint stores draft tensors
        # under `mtp.{N}.*` while the draft model constructs them as
        # `model.layers.{num_hidden_layers + N}.*` (num_hidden_layers is one
        # past the last rank-sliced MoE layer).  Map draft-layer prefixes back
        # to their `mtp.{N}` storage keys so FP8 detection works there too.
        rsm = self.rank_sliced_metadata
        if rsm is not None:
            last_moe = int(rsm["moe_layers"][1])
            m = re.match(r"(?:model\.)?layers\.(\d+)\b", prefix)
            if m is not None:
                idx = int(m.group(1))
                if idx > last_moe:
                    mtp_idx = idx - (last_moe + 1)
                    for cand in list(candidates):
                        cand_alt = re.sub(
                            r"(?:model\.)?layers\.\d+", f"mtp.{mtp_idx}", cand
                        )
                        candidates.append(cand_alt)

        # Embedded DSPark MTP draft model-level params (main_proj/main_norm
        # live in mtp.0; norm/hc_head/markov_head in the final mtp stage).
        # The draft registers them at model level, the checkpoint under
        # `mtp.{N}.*`, so map them for FP8/EXL3 detection too.
        if rsm is not None:
            last_stage = 2 if self.tensor_storage and any(
                k.startswith("mtp.2.") for k in self.tensor_storage
            ) else 0
            for cand in list(candidates):
                for mtp_idx in (0, last_stage):
                    if re.match(
                        r"(?:model\.)?(main_proj|main_norm)\b", cand
                    ):
                        cand_alt = re.sub(
                            r"(?:model\.)?(main_proj|main_norm)",
                            rf"mtp.{mtp_idx}.\1",
                            cand,
                        )
                        candidates.append(cand_alt)
                    if re.match(
                        r"(?:model\.)?(norm|hc_head_fn|hc_head_base|hc_head_scale|markov_head)\b",
                        cand,
                    ):
                        cand_alt = re.sub(
                            r"(?:model\.)?(norm|hc_head_fn|hc_head_base|hc_head_scale|markov_head)",
                            rf"mtp.{mtp_idx}.\1",
                            cand,
                        )
                        candidates.append(cand_alt)
    ...
    def _linear_prefix_is_fp8(self, prefix: str) -> bool:
        """True when the linear (or its packed sources) are stored as FP8.

        Storage entries are the raw checkpoint tensor names (e.g.
        ``layers.0.attn.wq_b.weight`` / ``layers.0.attn.wq_b.scale``).  A
        linear is FP8 when its weight tensor is float8 or its scale entry is
        stamped ``fp8``.  For merged linears (``fused_wqa_wkv`` -> wq_a/wkv,
        ``gate_up_proj`` -> w1/w3, ``fused_wkv_wgate`` -> wkv/wgate) resolve
        each packed source leaf via packed_modules_mapping.
        """
        if self._prefix_is_fp8(prefix):
            return True
        leaf = prefix.rsplit(".", 1)[-1]
        source_leaves = self.packed_modules_mapping.get(leaf)
        if not source_leaves:
            # Shared experts are stored as w1/w2/w3 in the checkpoint but
            # constructed as gate_up_proj/down_proj; gate_up_proj is already
            # resolved via packed_modules_mapping, down_proj needs w2.
            if leaf == "down_proj":
                source_leaves = ["w2"]
            else:
                return False
        base = prefix.rsplit(".", 1)[0] if "." in prefix else ""
        return all(
            self._prefix_is_fp8(f"{base}.{source}" if base else source)
            for source in source_leaves
        )

    def _prefix_is_fp8(self, prefix: str) -> bool:
        weight_entry = self._storage_entry(f"{prefix}.weight")
        if self._is_fp8_entry(weight_entry):
            return True
        scale_entry = self._storage_entry(f"{prefix}.scale")
        return self._is_fp8_entry(scale_entry)

    def _is_fp8_entry(self, entry: dict[str, Any] | None) -> bool:
        if entry is None:
            return False
        qf = entry.get("quant_format")
        if qf == "exl3":
            return False
        if qf == "fp8":
            return True
        ...
        return (
            dtype.startswith("float8")
            or dtype.endswith("e4m3fn")
            or dtype.endswith("e8m0fnu")
        )
...
class Exl3MoEParameter(BasevLLMParameter):
    ...
    def __new__(
        ...
        *,
        ...
        del num_experts, shard_ids, preallocate, checkpoint_tp, runtime_tp
        data = torch.empty(0, dtype=torch.uint8)
        return super().__new__(cls, data=data, weight_loader=weight_loader)
    ...
    def __init__(
        ...
        *,
        ...
        self.exl3_checkpoint_tp = int(checkpoint_tp)
        self.exl3_runtime_tp = int(runtime_tp)
        # For TP coalescing, the intermediate dimension index differs:
        # w13 trellis: dim=1, w2 trellis: dim=0, suh/svh: dim=0.
        self.exl3_concat_dim: int = 0
        # TP coalescing: track how many rank slices have been loaded per expert.
        self.exl3_slice_count: dict[tuple[int, str], int] = {}
        super().__init__(data=self.data, weight_loader=weight_loader)
    ...
    def load_exl3_weight(
        ...
        *,
        ...
        coalesce = (self.exl3_checkpoint_tp > self.exl3_runtime_tp
                    and self.exl3_concat_dim >= 0)
        concat_dim = max(self.exl3_concat_dim, 0)
        if self.exl3_backing is None:
            # When coalescing, allocate the full intermediate dimension.
            weight_shape = tuple(loaded_weight.shape)
            if coalesce:
                shape_list = list(weight_shape)
                shape_list[concat_dim] *= self.exl3_checkpoint_tp
                weight_shape = tuple(shape_list)
            prefix = (
        ...
        if coalesce:
            slice_idx = self.exl3_slice_count.get(key, 0)
            rank_slice_size = loaded_weight.shape[concat_dim]
            offset = slice_idx * rank_slice_size
            if target.shape[concat_dim] < offset + rank_slice_size:
                raise ValueError(
                    f"EXL3 rank-slice overflow: expert={expert_id}, "
                    f"shard={shard_id}, slice={slice_idx}, "
                    f"dim={concat_dim}, target={tuple(target.shape)}, "
                    f"loaded={tuple(loaded_weight.shape)}"
                )
            # Narrow along the concat dim and copy.
            src = loaded_weight
            dst = target.narrow(concat_dim, offset, rank_slice_size)
            dst.copy_(src, non_blocking=True)
            self.exl3_slice_count[key] = slice_idx + 1
            # Store the full coalesced slab so _rank_sliced_backing's
            # data_ptr check against backing[...] passes.
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        ckpt_tp = (int(self.quant_config.rank_sliced_metadata["tp"])
                   if rank_sliced else 1)
        rt_tp = layer.exl3_tp_size
        for prefix, shard_ids in (("w13", ("w1", "w3")), ("w2", ("w2",))):
            for suffix in ("suh", "svh", "trellis", "mcg", "mul1"):
                param = Exl3MoEParameter(
                    ...
                    checkpoint_tp=ckpt_tp,
                    runtime_tp=rt_tp,
                )
                # trellis + svh are coalesced (split across TP intermediate).
                # suh for w13 is hidden (not split); suh for w2 is
                # intermediate (split).  mcg/mul1 are scalars (not split).
                if suffix in ("mcg", "mul1"):
                    param.exl3_concat_dim = -1
                elif prefix == "w13":
                    if suffix == "suh":
                        param.exl3_concat_dim = -1  # hidden, not split
                    elif suffix == "trellis":
                        param.exl3_concat_dim = 1   # intermediate at dim 1
                else:  # w2
                    if suffix == "svh":
                        param.exl3_concat_dim = -1  # hidden, not split
    ...
    def _prepare_rank_sliced_weights(self, layer: RoutedExperts) -> None:
        ...
        checkpoint_tp = int(self.quant_config.rank_sliced_metadata["tp"])
        rank_intermediate_size = intermediate_size // checkpoint_tp
        layer.exl3_rank_intermediate_size = rank_intermediate_size
        # After TP coalescing (checkpoint_tp > runtime_tp), the slabs have the
        # FULL intermediate dimension.  Check against the actual slab shape.
        slab_intermediate = w13.shape[3] * 16
        # The rotation matrices (svh, down_svh, down_suh) are also coalesced
        # to full size when checkpoint_tp > runtime_tp.
        rot_intermediate = slab_intermediate if checkpoint_tp > layer.exl3_tp_size else rank_intermediate_size
        if checkpoint_tp > layer.exl3_tp_size:
            layer.exl3_rank_intermediate_size = slab_intermediate
        expected_w13 = (
            2,
            num_experts,
            hidden_size // 16,
            slab_intermediate // 16,
            16 * bits,
        )
        expected_w2 = (
            num_experts,
        ...
        intermediate_rotations = torch.empty(
            (num_experts, 3 * rot_intermediate),
            dtype=torch.float16,
            device=w13.device,
        )
        intermediate_rotations[:, :rot_intermediate].copy_(gate_svh)
        intermediate_rotations[:, rot_intermediate : 2 * rot_intermediate].copy_(
            up_svh
        )
        intermediate_rotations[:, 2 * rot_intermediate :].copy_(down_suh)
        tile_config = self._trellis_tile_config(hidden_size, rot_intermediate)
    ...
    def _rank_sliced_runtime(
        ...
        def _plan_with_scratch(plan_max_tokens: int, plan_block_m: int):
            caps = api.Caps(
                ...
                num_experts=int(layer.local_num_experts),
                hidden_size=int(layer.exl3_hidden_size),
                intermediate_size=int(getattr(layer, "exl3_rank_intermediate_size", layer.exl3_intermediate_size_per_partition)),
