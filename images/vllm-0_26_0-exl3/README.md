...
| 15 | Strip `)Skip` artifact | `patch_strip_skip_artifact.py` | DSPark spec-decode occasionally injects the standalone `)Skip` token (id 83480) mid-reply. Strips it from chat responses at the OpenAI serving layer (streaming + non-streaming) |
